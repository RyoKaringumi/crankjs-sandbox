...
/*
keyプロパティは特殊扱いされており、仕様は基本的にReactと同一である。

そのため、リストで要素を返す場合、警告などは出ないものの、keyをつけるのが良さそうです。

また、keyを利用し、要素を入れ替える事が正しく実現出来ている事を確認しました。

検証方法として、ItemコンポーネントのrandomIdが要素を入れ替える事で、適切に維持されながら入れ替わっている事が確認できます。

keyプロパティを一時的に無効化する事で、実質的にindexをkeyとして暗黙的に利用するため、要素を入れ替えてもrandomIdは元の位置のまま入れ替わりません。

注意点として、位置を移動させる場合でも、そうでない場合でも、{} of thisが再度実行されます。
*/
